function __fish_preexec_wakatime --on-event fish_preexec
  ...
  test -d $PWD/.git; and begin
    if not test -f $PWD/.wakatime-project
      set project (git -C $PWD remote get-url (git -C $PWD remote | head -1))
  ...
  wakatime --write --plugin "fish-wakatime/0.0.1" --entity-type app --project "$project" --entity (echo $argv | cut -d ' ' -f1) 2>&1 > /dev/null &
