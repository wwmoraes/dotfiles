	...
	home-manager.sharedModules = [({ config, ... }: {
			programs.kitty = {
				...
				font = {
					...
					name = "FiraCode Nerd Font Mono"; ## fc-list : family | sort | uniq
					size = 16.0;
				};
				keybindings = {
				...
				extraConfig = pkgs.lib.local.unindent ''
			...
			xdg.configFile."kitty/unicode-input-favorites.conf".text = pkgs.lib.local.unindent ''
			...
			xdg.configFile."fish/conf.d/kitty.fish".text = pkgs.lib.local.unindentTrim ''
