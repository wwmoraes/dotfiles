...
in {
	...
	home-manager.sharedModules = [({ config, ... }: {
		programs.helix = let
			defaultLanguages = builtins.fromTOML (builtins.readFile ./helix-languages.toml);
			languageAttrs = builtins.foldl' (attrs: entry: attrs // {
		...
		in {
			...
			languages = (builtins.removeAttrs defaultLanguages ["language"]) // {
				language = lib.mapAttrsToList (k: v: { name = k; } // v) (lib.recursiveUpdate (lib.recursiveUpdate languageAttrs (
					## globally change indent to tabs unless unsupported by a language
					builtins.listToAttrs (map (name: {
						name = name;
						value = {
							indent = { tab-width = 2;	unit = "\t"; };
						};
					}) (builtins.attrNames (builtins.removeAttrs languageAttrs ["yaml"])))
				)) {
					env = {
