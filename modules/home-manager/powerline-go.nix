...
in {
    ...
    programs.powerline-go = {
      ...
      pathAliases = mkOption {
        ...
        description = ''
          Pairs of full-path and corresponding desired short name.
        '';
    ...
    programs.zsh.initContent = mkIf cfg.enableZshIntegration (mkBefore (pkgs.lib.local.unindent ''
    ...
    ''));
		...
		programs.fish.functions.fish_prompt.body = mkIf cfg.enableFishIntegration (mkAfter (pkgs.lib.local.unindent ''
		...
		xdg.configFile."powerline-go/config.json" = {
			enable = config.programs.powerline-go.enable;
			text = builtins.toJSON (builtins.removeAttrs (config.programs.powerline-go.settings // {
				modules = config.programs.powerline-go.modules;
				modules-right = config.programs.powerline-go.modulesRight;
				path-aliases = config.programs.powerline-go.pathAliases;
			}) ["shell"]);
		};
	};
}
