...
function obj:search(pat)
  ...
  for repo, v in pairs(self.repos) do
    if v.data then
      ...
      self.logger.ef("Repository data for '%s' not available - call spoon.SpoonInstall:updateRepo('%s'), then try again."
        , repo, repo)
    end
  end
  return res
...
function obj:_installSpoonFromZipURLgetCallback(urlparts, callback, status, body, headers)
  ...
  if (status < 100) or (status >= 400) then
    ...
    if tmpdir then
      ...
      local output = _x(string.format("/usr/bin/unzip -l %s '*.spoon/' | /usr/bin/awk '$NF ~ /\\.spoon\\/$/ { print $NF }' | /usr/bin/wc -l"
        , outfile),
      ...
      if output then
        if (tonumber(output) or 0) == 1 then
          ...
          self.logger.ef("The downloaded zip file %s is invalid - it should contain exactly one spoon. Leaving it in place for your examination."
            , outfile)
        end
      end
    end
