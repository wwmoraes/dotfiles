...
while [ $# -gt 0 ]; do
	...
	case ${i} in
		...
		--profile-name|-p)
			...
			if [ "$1" != system ]; then
				mkdir -m 0755 /nix/var/nix/profiles/system-profiles
...
if [ "${action}" = switch ] || [ "${action}" = build ] || [ "${action}" = check ] || [ "${action}" = changelog ]; then
	...
	if [ -z "${flake}" ]; then
		systemConfig="$(sudo --user="${SUDO_USER}" env HOME="$(eval echo "~${SUDO_USER}")" nix-build '<darwin>' "${extraBuildFlags[@]}" -A system)"
	else
		systemConfig=$(sudo --user="${SUDO_USER}" env HOME="$(eval echo "~${SUDO_USER}")" nix "${flakeFlags[@]}" build --print-out-paths "${extraBuildFlags[@]}" "${extraLockFlags[@]}" -- "${flake}#${flakeAttr}.system")
	fi
fi
