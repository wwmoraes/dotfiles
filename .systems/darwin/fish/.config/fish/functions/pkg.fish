...
function pkg -a cmd -w pkgutil -d "MacOS package management made easy"
  ...
  switch "$cmd"
    ...
    case uninstall
      ...
      set -l files (pkgutil --only-files --files $package)
      ...
      echo "removing files..."
      # TODO make it space-safe!
      sudo rm -f (echo $files | xargs -n1 -I % echo /%)

      echo "removing empty directories..."
      set -l err (mkfifo (mktemp -u))
      # TODO make it space-safe!
      sudo rmdir -p -v (echo $files | xargs dirname | sort -u | xargs -I % echo /%) 2>err | grep -v "No such file or directory\|Directory not empty" err
      rm -f $err
